...
export default function HomeScreen() {
  const {
    ...
    playSound,
    stopSound,
  } = useAudioRecording(recordingSettings as Audio.RecordingOptions);
  ...
  const renderTypeCard = ({
    ...
    item: { name: string; icon: string; effect: string };
  }) => {
    return (
      <TouchableOpacity
        style={styles.typeCard}
        onPress={() => handleCardPress(item.name, item.effect)}
      >
        <Text>{item.name}</Text>
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.header}>
        Yak Bak <Text style={styles.selectedTypeText}>{selectedType}</Text>
      </Text>
      <View
        style={[
          styles.iconContainer,
          { backgroundColor: "#fee9d7", borderRadius: 100 },
        ]}
      >
        <TouchableOpacity
      ...
      <TouchableOpacity
        ...
        onPress={() => playSound(playbackRate, selectedEffect)}
      >
